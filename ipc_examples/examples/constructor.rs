...
fn main() {
    ...
    let timestamp = K::new_timestamp_list(
        vec![NaiveDate::from_ymd_opt(2009, 5, 28)
            .unwrap()
            .and_hms_nano_opt(17, 30, 0, 188906523)
            .unwrap()
    ...
    let timespan_list = K::new_timespan_list(
        vec![
            Duration::minutes(30),
            Duration::seconds(15),
            Duration::nanoseconds(123456),
        ],
        qattribute::NONE,
    );
    ...
    let datetime_list = K::new_datetime_list(
        vec![
            NaiveDate::from_ymd_opt(2017, 4, 9)
                .unwrap()
                .and_hms_milli_opt(16, 51, 30, 97)
                .unwrap()
                .and_local_timezone(Utc)
                .unwrap(),
            NaiveDate::from_ymd_opt(2020, 11, 3)
                .unwrap()
                .and_hms_milli_opt(10, 20, 8, 103)
                .unwrap()
                .and_local_timezone(Utc)
                .unwrap(),
        ],
    ...
    let compound_list =
        K::new_compound_list(vec![short_list, string, symbol_list, long, datetime_list]);
    println!("{}", compound_list);

    let keys = K::new_int_list(vec![1, 2, 3], qattribute::SORTED);
    let values = K::new_symbol_list(
